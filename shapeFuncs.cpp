...
double distanceBetween(Point p, Point q) {
	double distance = (sqrt((pow((p.x-q.x),2))+pow((p.y-q.y),2)));
...
void initPoint(struct Point *p, double xVal, double yVal) {
  	//pre: gets the address of a struct point and the x and y elements of the point
	////post: input values of x and y into **p
	//**p = {xVal,yVal};
	(*p).x = xVal;
	(*p).y = yVal;
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance) {
	if (pointsApproxEqual(b1.ul,b2.ul) && (fabs(b1.height-b2.height)<tolerance) && (fabs(b1.width-b2.width) <tolerance))
		return true; 
	else
